...
namespace AppDesktop.Ticket_Venta
{
    public class CrearTicket
    {
        ...
        public void EncabezadoVenta()
        {
            ...
            linea.AppendLine("PELICULA              |SALA|HORA|IMPORTE");
        }
        ...
        public void AgregaArticulo(string tipo_bol, string nom_peli, string hora_fun, decimal importe)
        {
            //Valida que cant precio e importe esten dentro del rango.
            if (nom_peli.ToString().Length <= 5 && hora_fun.ToString().Length <= 7 && importe.ToString().Length <= 8)
            {
                ...
                if (tipo_bol.Length > 20)
                {
                    //Colocar la cantidad a la derecha.
                    nroEspacios = (5 - nom_peli.ToString().Length);
                    espacios = "";
                    for (int i = 0; i < nroEspacios; i++)
                    {
                        espacios += " ";//Generamos los espacios necesarios para alinear a la derecha
                    }
                    elemento += espacios + nom_peli.ToString();//agregamos la cantidad con los espacios

                    //Colocar el precio a la derecha.
                    nroEspacios = (7 - hora_fun.ToString().Length);
                    espacios = "";
                    for (int i = 0; i < nroEspacios; i++)
                    {
                    ...
                    elemento += espacios + hora_fun.ToString();//Agregamos el precio a la variable elemento
                    ...
                    for (int longitudTexto = tipo_bol.Length; longitudTexto > 20; longitudTexto -= 20)
                    {
                        if (bandera == false)//si es false o la primera linea en recorrerer, continuar...
                        {
                            ...
                            linea.AppendLine(tipo_bol.Substring(caracterActual, 20) + elemento);
                            bandera = true;//cambiamos su valor a verdadero
                        }
                        else
                            linea.AppendLine(tipo_bol.Substring(caracterActual, 20));//Solo agrega el nombre del articulo
                    ...
                    linea.AppendLine(tipo_bol.Substring(caracterActual, tipo_bol.Length - caracterActual));

                }
                else //Si no es mayor solo agregarlo, sin dar saltos de lineas
                {
                    for (int i = 0; i < (20 - tipo_bol.Length); i++)
                    {
                        espacios += " "; //Agrega espacios para completar los 20 caracteres
                    }
                    elemento = tipo_bol + espacios;

                    //Colocar la cantidad a la derecha.
                    nroEspacios = (5 - nom_peli.ToString().Length);// +(20 - elemento.Length);
                    espacios = "";
                    for (int i = 0; i < nroEspacios; i++)
                    {
                        espacios += " ";
                    }
                    elemento += espacios + nom_peli.ToString();
                    ...
                    elemento += espacios + hora_fun.ToString();
